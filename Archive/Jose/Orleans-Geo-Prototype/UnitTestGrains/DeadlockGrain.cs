...
namespace UnitTestGrains
{
    ...
    {
        internal static Task CallNext(List<Tuple<long, bool>> callChain, int currCallIndex)
        {
            ...
            if (next.Item2)
            {
                IDeadlockNonReentrantGrain nextGrain = DeadlockNonReentrantGrainFactory.GetGrain(next.Item1);
                if (call_1)
            ...
            {
                IDeadlockReentrantGrain nextGrain = DeadlockReentrantGrainFactory.GetGrain(next.Item1);
                if (call_1)
                    return nextGrain.CallNext_1(callChain, currCallIndex + 1);
                else
                    return nextGrain.CallNext_2(callChain, currCallIndex + 1);
    ...
    public class DeadlockNonReentrantGrain : GrainBase, IDeadlockNonReentrantGrain
    {
        ...
        public Task CallNext_1(List<Tuple<long, bool>> callChain, int currCallIndex)
        {
            ...
            return DeadlockGrain.CallNext(callChain, currCallIndex);
        }
    ...
    [Reentrant]
    public class DeadlockReentrantGrain : GrainBase, IDeadlockReentrantGrain
    {
