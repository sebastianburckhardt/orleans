...
For example, in \emph{functional-reactive programming (FRP)}, we express views as signals that depend on event streams, and define them using a vocabulary of functional operators \cite{frp-firstprinciples,frp-frtime,elm,\cite{afp}}. Another approach, which we call \emph{model-view computation}, is to define views as the result of a regular functional or imperative computation that depends on the model state \cite{alive,react}. In either case, the ability to propagate small changes selectively and efficiently is key. This can be achieved not only at the language and compiler level, but also by clever runtime algorithms (such as virtual DOMs \cite{react,alive}, dependency detection via page faults \cite{camil}, or self-adjusting computation \cite{acar-ahmed-blume-POPL08}).
