...
{
    [Reentrant]
    public class ReentrantGrain : GrainBase, IReentrantGrain
    {
    ...
    public class NonRentrantGrain : GrainBase, INonReentrantGrain
    {
    ...
    public class UnorderedNonRentrantGrain : GrainBase, IUnorderedNonReentrantGrain
    {
    ...
    [Reentrant]
    public class ReentrantSelfManagedGrain1 : GrainBase, IReentrantSelfManagedGrain
    {
        ...
        public override Task ActivateAsync()
        {
            logger = GetLogger(this.GetType().Name + "-" + this.GetPrimaryKeyLong());
            return TaskDone.Done;
        }
        ...
        public Task Ping(int seconds)
        {
            ...
            IReentrantSelfManagedGrain otherGrain = ReentrantSelfManagedGrainFactory.GetGrain(destination);
    ...
    public class NonReentrantSelfManagedGrain1 : GrainBase, INonReentrantSelfManagedGrain
    {
        private long destination;
        private OrleansLogger logger;
        ...
        public Task Ping(int seconds)
        {
            ...
            logger.Info("Before GetCounter - OtherId={0}", destination);
            INonReentrantSelfManagedGrain otherGrain = NonReentrantSelfManagedGrainFactory.GetGrain(destination);
            var ctr = otherGrain.GetCounter();
            logger.Info("After GetCounter() - returning promise");
            return ctr;
        }
    ...
    [Reentrant]
    public class FanOutGrain : GrainBase, IFanOutGrain
    {
        private OrleansLogger logger;
        ...
        private async Task<IReentrantTaskGrain[]> InitTaskGrains_Reentrant(int offset, int num)
        {
            ...
            {
                int idx = offset + i;
                IReentrantTaskGrain grain = ReentrantTaskGrainFactory.GetGrain(idx);
        ...
        private async Task<INonReentrantTaskGrain[]> InitTaskGrains_NonReentrant(int offset, int num)
        {
            ...
            {
                int idx = offset + i;
                INonReentrantTaskGrain grain = NonReentrantTaskGrainFactory.GetGrain(idx);
                fanOutGrains[i] = grain;
                int next = offset + ((i + 1) % num);
                Task promise = grain.SetDestination(next);
                promises.Add(promise);
    ...
    [Reentrant]
    public class FanOutACGrain : GrainBase, IFanOutACGrain
    {
        private OrleansLogger logger;
        private static readonly TimeSpan OneSecond = TimeSpan.FromSeconds(1);
        ...
        private async Task<IReentrantSelfManagedGrain[]> InitACGrains_Reentrant(int offset, int num)
        {
            ...
            {
                int idx = offset + i;
                var grain = ReentrantSelfManagedGrainFactory.GetGrain(idx);
                fanOutGrains[i] = grain;
                int next = offset + ((i + 1) % num);
                Task promise = grain.SetDestination(next);
                promises.Add(promise);
        ...
        private async Task<INonReentrantSelfManagedGrain[]> InitACGrains_NonReentrant(int offset, int num)
        {
            ...
            {
                int idx = offset + i;
                var grain = NonReentrantSelfManagedGrainFactory.GetGrain(idx);
                fanOutGrains[i] = grain;
                int next = offset + ((i + 1) % num);
                Task promise = grain.SetDestination(next);
                promises.Add(promise);
    ...
    [Reentrant]
    public class ReentrantTaskGrain : GrainBase, IReentrantTaskGrain
    {
        private OrleansLogger logger;
        ...
        public async Task Ping(TimeSpan wait)
        {
            ...
            var otherGrain = ReentrantTaskGrainFactory.GetGrain(otherId);
    ...
    public class NonReentrantTaskGrain : GrainBase, INonReentrantTaskGrain
    {
        private OrleansLogger logger;
        private long otherId;
        private int count;
        ...
        public async Task Ping(TimeSpan wait)
        {
            ...
            await Task.Delay(wait);
            logger.Info("Before GetCounter - OtherId={0}", otherId);
            var otherGrain = NonReentrantTaskGrainFactory.GetGrain(otherId);
            var ctr = await otherGrain.GetCounter();
            logger.Info("After GetCounter() - got value={0}", ctr);
        }
