...
namespace Orleans
{
    internal class ByteArrayBuilder
    {
        ...
        private uint[] tempUIntArray = new uint[1];
        private short[] tempShortArray = new short[1];
        private ushort[] tempUShortArray = new ushort[1];
        private long[] tempLongArray = new long[1];
        private ulong[] tempULongArray = new ulong[1];
        private double[] tempDoubleArray = new double[1];
        private float[] tempFloatArray = new float[1];
        ...
        {
            EnsureRoomFor(1);
            currentBuffer[currentOffset++] = Convert.ToByte(c);
            return this;
        }
        ...
        public ByteArrayBuilder Append(string s)
        {
            ...
            if ((currentOffset + count <= bufferSize) && (currentBuffer != null))
            {
                currentOffset += Encoding.UTF8.GetBytes(s, 0, s.Length, currentBuffer, currentOffset);
            }
            // Big enough to justify its own buffer
            else if (((currentOffset > MINIMUM_BUFFER_SIZE) && (count > MINIMUM_BUFFER_SIZE)) || (count >= bufferSize))
            {
                Grow();
                completedBuffers.Add(new ArraySegment<byte>(Encoding.UTF8.GetBytes(s)));
            }
            else
            {
                // Fits in this buffer plus the next
