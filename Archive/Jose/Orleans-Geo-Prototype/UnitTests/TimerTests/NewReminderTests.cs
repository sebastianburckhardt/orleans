...
namespace UnitTests.TimerTests
{
    ...
    public class NewReminderTests : UnitTestBase
    {
        ...
        // [mlr] for a small-scale test such as this, the periods ought to be really precisely triggered.
                ...
                DateTime startTestAt)
        {
            ...
            var delay = (startTestAt - DateTime.UtcNow) + TimeSpan.FromSeconds(RNG.NextDouble() * reminderPeriod.TotalSeconds);
                ...
                string reminderName)
        {
            ...
            // [mlr] now that the reminder is stopped, wait for a whole period.
            await Task.Delay(stats1.RequestedPeriod);
            var stats2 = await grain.GetTestStats(reminderName);
            // [mlr] the grain should not have gotten any additional ticks from our reminder.
                ...
                string reminderName)
        {
            ...
            var tasks =
                grains.Select(
                    g =>
                        this.FinishTest(g, reminderName))
                    .ToArray();
            await Task.WhenAll(tasks);
            var results =
                tasks.Select(
                ...
                int periodMultiplier)
        {
            ...
            // [mlr] the test length should last at least as long as two reminder periods.
            ...
            // [mlr] based on observation, it takes approximately 30 sec. to create 100000 timers on the build machines,
