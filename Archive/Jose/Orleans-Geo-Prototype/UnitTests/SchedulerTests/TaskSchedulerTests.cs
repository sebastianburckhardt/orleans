...
namespace UnitTests.SchedulerTests
{
    ...
    public class TaskSchedulerTests
    {
        ...
        [TestMethod, TestCategory("Nightly"), TestCategory("TaskScheduler")]
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("TaskScheduler")]
        ...
        public void Sched_Task_TaskWorkItem_CurrentScheduler()
        {
            ...
            ResultHandle resultHandle0 = new ResultHandle();
            ...
            scheduler.QueueWorkItem(new ClosureWorkItem(() =>
            {
                try
                {
                    ...
                    resultHandle0.Done = true;
                }
                catch (Exception exc)
                {
                    resultHandle0.Exception = exc;
                }
            }), context);

            resultHandle0.WaitForFinished(TimeSpan.FromSeconds(1));
            Assert.IsTrue(resultHandle0.Exception == null, "Task-0 should not throw exception: " + resultHandle0.Exception);
            Assert.IsTrue(resultHandle0.Done, "Task-0 completed");

            Assert.IsNotNull(t1, "Task-1 started");
            resultHandle1.WaitForFinished(TimeSpan.FromSeconds(1));
            Assert.IsTrue(t1.IsCompleted, "Task-1 completed");
            Assert.IsFalse(t1.IsFaulted, "Task-1 faulted: " + t1.Exception);
            Assert.IsTrue(resultHandle1.Done, "Task-1 completed");
        }
        ...
        // This test case demonstrates incorrect case - test case Sched_Task_NewTask_ContinueWith_Wrapped is the way it works in practice
        //[TestMethod, TestCategory("Failures"), TestCategory("TaskScheduler"), ]
        //public void Sched_Task_NewTask_ContinueWith()
        //{
        ...
        //    });
        //    t0.Start(scheduler);
        //    t1.Wait(TimeSpan.FromSeconds(15));
        //}
        ...
        public void Sched_Task_NewTask_ContinueWith_Wrapped_OrleansTaskScheduler()
        {
            ...
            OrleansTaskScheduler scheduler = OrleansTaskScheduler.InitializeSchedulerForTesting(rootContext);
        ...
        public void Sched_AC_RequestContext_StartNew_ContinueWith()
        {
            ...
            AsyncCompletion t0 = AsyncCompletion.StartNew(() =>
            {
                Console.WriteLine("#0 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
                    SynchronizationContext.Current, TaskScheduler.Current);
                ...
                AsyncCompletion t1 = AsyncCompletion.StartNew(() =>
                {
                    Console.WriteLine("#1 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
                        SynchronizationContext.Current, TaskScheduler.Current);
                    Assert.AreEqual(val, RequestContext.Get(key), "RequestContext.Get #1");
                });
                AsyncCompletion t2 = t1.ContinueWith(() =>
                {
                    Console.WriteLine("#2 - new Task - SynchronizationContext.Current={0} TaskScheduler.Current={1}",
                        SynchronizationContext.Current, TaskScheduler.Current);
                    Assert.AreEqual(val, RequestContext.Get(key), "RequestContext.Get #2");
        ...
        internal static void InitSchedulerLogging()
        {
            ...
            var traceLevels = new[]
            {
                Tuple.Create("Scheduler", OrleansLogger.Severity.Verbose3),
                Tuple.Create("Scheduler.WorkerPoolThread", OrleansLogger.Severity.Verbose2),
            };
