...
namespace LocalSimulation
{
    ...
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            var servers = new GeoOrleans.Benchmarks.Server[numservers];
            var urlpath = new string[numservers];

            for (int i = 0; i < numservers; i++)
            {
                string servername = "server" + i;
                servers[i] = new GeoOrleans.Benchmarks.Server(
            ...
            var benchmarkconsole = new GeoOrleans.Benchmarks.Console(
            ...
            while (true)
            {
                ...
                foreach (var x in scenarios.Value.Value)
                {
                    ...
                    Console.WriteLine(GeoOrleans.Benchmarks.Common.Util.PrintStats(simulator.Stats));
