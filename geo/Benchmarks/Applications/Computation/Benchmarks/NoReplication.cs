...
namespace GeoOrleans.Benchmarks.Computation.Benchmark
{
    ...
    public class NoReplicationComputation : IScenario
    {
        ...
        public async Task<string> RobotScript(IRobotContext context, int robotnumber, string parameters)
        {
            ...
            GeoOrleans.Runtime.Common.Util.Assert(totReads == (percentRead * totOps / 100), "Incorrect Number Reads " + totReads);
            GeoOrleans.Runtime.Common.Util.Assert(totWrites == (percentWrite * totOps / 100), "Incorrect Number Writes " + totWrites);
    ...
    public class HttpRequestComputation : IHttpRequest
    {
        ...
        public string Signature
        {
            get
            {
                ...
                {
                    GeoOrleans.Runtime.Common.Util.Assert(payload != null, "Cannot have WRITE type and null payload");
        ...
        public async Task<string> ProcessRequestOnServer()
        {
            ...
            {
                GeoOrleans.Runtime.Common.Util.Assert(requestType == ComputationRequestT.WRITE_SYNC);
