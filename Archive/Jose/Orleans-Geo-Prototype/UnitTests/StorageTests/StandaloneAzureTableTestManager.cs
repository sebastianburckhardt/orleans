        ...
        internal static StandaloneAzureTableTestManager GetManager(string deploymentId, string storageConnectionString)
        {
            lock (classLock)
            {
                if (singleton == null)
                {
                    ...
                    try
                    {
                        singleton.InitTableAsync().WaitWithThrow(AzureTableDefaultPolicies.TableCreation_TIMEOUT);
                    }
                    catch (TimeoutException te)
                    {
                        singleton.logger.Error(ErrorCode.AzureTable_38, String.Format("Unable to create or connect to the Azure table in {0} {1}", AzureTableDefaultPolicies.TableCreation_TIMEOUT, te));
                        throw;
                    }
                    catch (Exception ex)
                    {
