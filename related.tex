...
% Because actors can be modified independently on different machines, our applications are \emph{exochronous} (multiple inputs to the dependency graph) as opposed to \emph{endochronous}.  
...
\paragraph{AmbientTalk/R}
AmbientTalkR also introduces reactive programming into the actor model. The focus there lies on providing reactivity within a mobile ad-hoc network. So-called \emph{ambient behaviours} can be exposed, which allow actors to share behaviours using an intentional description of the behaviour. A decentralized pub/sub mechanism then makes sure proximate actors can connect to each other through these behaviours. No glitch freedom is guaranteed for the ambient behaviours at all, since that would not be suited for an ambient network.
