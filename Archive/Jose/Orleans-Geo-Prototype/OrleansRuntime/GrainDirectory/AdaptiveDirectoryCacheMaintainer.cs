...
namespace Orleans.Runtime.GrainDirectory
{
    ...
    {
        private static readonly TimeSpan SLEEP_TIME_BETWEEN_REFRESHES = TimeSpan.FromMinutes(1); // 1 minute -- Halo only, since min TTL for them is 10 minutes; this should be something like minTTL/4
        ...
        protected override void Run()
        {
            while (router.running)
            {
                ...
                // recheck every X seconds (//TODO: make it a configurable parameter)
        ...
        private void SendBatchCacheRefreshRequests(Dictionary<SiloAddress, List<GrainId>> refreshRequests)
        {
            foreach (SiloAddress silo in refreshRequests.Keys)
            {
                ...
                Action todo = () => AsyncValue.FromTask(
                    validator.LookUpMany(cachedGrainAndETagList)).
                        SystemContinueWith(response => ProcessCacheRefreshResponse(capture, cachedGrainAndETagList, response)).Ignore();
                router.Scheduler.QueueAction(todo, router.CacheValidator.SchedulingContext).Ignore();

                //// Send the full request in batches of 500 items
                //for (int i = 0; i < cachedGrainAndETagList.Count; i += 500)
                //{
                //    var thisRequestList = cachedGrainAndETagList.GetRange(i, Math.Min(500, cachedGrainAndETagList.Count - i));
                //    Action todo = () => validator.LookUpMany(capture, thisRequestList).
                //                            FastSystemContinueWith(response => ProcessCacheRefreshResponse(capture, thisRequestList, response)).Ignore();
                //    router.Scheduler.QueueAction(todo, router.CacheValidator.SchedulingContext).Ignore();
                //}
        ...
        private void ProcessCacheRefreshResponse(SiloAddress silo, List<Tuple<GrainId, int>> refreshRequest, List<Tuple<GrainId, int, List<Tuple<SiloAddress, ActivationId>>>> refreshResponse)
        {
            if (log.IsVerbose2) log.Verbose2("Silo {0} received ProcessCacheRefreshResponse. #Request entries {1}, #response entries {2}.", router.MyAddress, refreshRequest.Count, refreshResponse.Count);  
            int cnt1 = 0, cnt2 = 0, cnt3 = 0;
            AdaptiveGrainDirectoryCache<List<Tuple<SiloAddress, ActivationId>>> cacheRef = cache as AdaptiveGrainDirectoryCache<List<Tuple<SiloAddress, ActivationId>>>;
            ...
            foreach (Tuple<GrainId, int, List<Tuple<SiloAddress, ActivationId>>> tuple in refreshResponse)
            {
                if (tuple.Item3 != null)
                {
                    ...
                    //((AdaptiveGrainDirectoryCache<List<Tuple<SiloAddress, ActivationId>>>)router.DirectoryCache).AddOrUpdate(tuple.Item1, tuple.Item3, tuple.Item2);
                ...
                else
                {
                    ...
                    // Validate that the generation number in the request and the response are equal (TODO: make it faster!)
