...
namespace UnitTests.SerializerTests
{
    ...
    public class BuiltInSerializerTests
    {
        ...
        public void Serialize_ArrayOfArrays()
        {
            ...
            var source4 = new IAggregatorGrain[3][];
            source4[0] = new IAggregatorGrain[2];
            source4[1] = new IAggregatorGrain[3];
            source4[2] = new IAggregatorGrain[1];
            source4[0][0] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            source4[0][1] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            source4[1][0] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            source4[1][1] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            source4[1][2] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            source4[2][0] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
            deserialized = OrleansSerializationLoop(source4);
            ValidateArrayOfArrays(source4, deserialized, "grain reference");

            var source5 = new IAggregatorGrain[32][];
            for (int i = 0; i < source5.Length; i++)
            {
                source5[i] = new IAggregatorGrain[64];
                for (int j = 0; j < source5[i].Length; j++)
                {
                    source5[i][j] = AggregatorGrainFactory.Cast(GrainReference.FromGrainId(GrainId.NewId()));
                }
            }
        ...
        {
            GrainBase input = new EchoTaskGrain();
        ...
        //internal class SerializeListTester : OutgoingMessageSender
        //{
        //    protected override bool PrepareMessageForSend(Message wmsg) { return true; }

        //    protected override bool GetSendingSocket(Message wmsg, out Socket sock, out SiloAddress targetSilo, out string error)
        //    {
        //        // Just a random Socket, SiloAddress, and error. Never used.
        //        sock = new Socket(System.Net.Sockets.SocketType.Unknown, System.Net.Sockets.ProtocolType.Unknown);
        //        targetSilo = SiloAddress.Zero;
        //        error = "";
        //        return true;
        //    }
