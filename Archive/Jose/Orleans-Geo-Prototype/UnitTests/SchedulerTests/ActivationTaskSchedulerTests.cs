...
{
    [TestClass]
    public class ActivationTaskSchedulerTests
    {
        ...
        public ActivationTaskSchedulerTests()
        {
        ...
        public void TestInit()
        {
            ...
            masterScheduler = OrleansTaskScheduler.InitializeSchedulerForTesting(context);
        }
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Scheduler")]
        public void ActivationSched_ContinueWith_1_Test()
        {
            TaskScheduler scheduler = masterScheduler.GetWorkItemGroup(context).TaskRunner;
            ...
            Task wrapper = new Task(() =>
            {
                ...
                Task task4 = task3.ContinueWith(task => { n = n - 2; Console.WriteLine("===> 4"); result.Done = true; });
                // ReSharper restore AccessToModifiedClosure
            ...
            Assert.IsTrue(result.WaitForFinished(TimeSpan.FromSeconds(2)));
            Assert.IsTrue(n != 0, "Work items did not get executed");
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Scheduler")]
        public void ActivationSched_FromAsync()
        {
            TaskScheduler scheduler = masterScheduler.GetWorkItemGroup(context).TaskRunner;

            int val = -23;
            Task promise = null;
            AC_FromAsync.SimpleAsyncResult result = null;
            ResultHandle finish = new ResultHandle();
            Task wrapper = new Task(() =>
            {
                Assert.AreEqual(scheduler, TaskScheduler.Current, "TaskScheduler.Current=" + TaskScheduler.Current);
                result = (AC_FromAsync.SimpleAsyncResult)AC_FromAsync.SimpleAsyncResult.BeginOperation(null);
                promise = Task.Factory.FromAsync(result, res => { val = AC_FromAsync.SimpleAsyncResult.EndOperation(res); });
                finish.Done = true;
            });
            wrapper.Start(scheduler);
            finish.WaitForFinished(TimeSpan.FromSeconds(2));
            Assert.AreEqual(-23, val, "Value was overwritten prematurely");
            Assert.IsFalse(promise.IsCompleted, "Promise status is incorrect " + promise);
            result.Complete(42);
            bool ok = promise.Wait(TimeSpan.FromSeconds(1));
            Assert.IsTrue(ok, "Finished OK");
            Assert.AreEqual(42, val, "Value was not overwritten");
        }

        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Scheduler")]
        public void ActivationSched_FromAsync_Failure()
        {
            TaskScheduler scheduler = masterScheduler.GetWorkItemGroup(context).TaskRunner;
            ...
            Assert.AreEqual(-23, val, "Value was overwritten prematurely");
            Assert.IsFalse(promise.IsCompleted, "Promise status is incorrect " + promise);
            result.Fail(new Exception(errorMsg));
            try
            {
                bool ok = promise.Wait(TimeSpan.FromSeconds(1));
            ...
            {
                Assert.IsTrue(promise.IsFaulted, "Promise status is incorrect " + promise);
                Exception exc = ae.GetBaseException();
                Assert.AreEqual(errorMsg, exc.Message, "Wrong error message");
            }
            catch (Exception ex2)
            {
        ...
        public void ActivationSched_Turn_Execution_Order_Loop()
        {
            TaskScheduler scheduler = masterScheduler.GetWorkItemGroup(context).TaskRunner;
            ...
            ResultHandle[] resultHandles = new ResultHandle[NumChains];
            ...
            for (int i = 0; i < NumChains; i++)
            {
                ...
                int sleepTime = rng.Next(100);
                resultHandles[i] = new ResultHandle();
                ...
                taskChainEnds[chainNum] = task.ContinueWith(t =>
                {
                    ...
                    resultHandles[chainNum].Done = true;
                }, scheduler);
            }
            ...
            for (int i = 0; i < NumChains; i++)
            {
                ...
                bool ok = resultHandles[i].WaitForFinished(waitCheckTime);
                ...
                Assert.IsTrue(resultHandles[i].Done, "ResultHandle for Task chain " + i + " should be Done");
            }
        }
        ...
        [TestMethod, TestCategory("Nightly"), TestCategory("Scheduler")]
        public async Task ActivationSched_Test1_AC_CW()
        {
            TaskScheduler scheduler = masterScheduler.GetWorkItemGroup(context).TaskRunner;

            NonReentrentStressGrainWithoutState grain = new NonReentrentStressGrainWithoutState();
            await grain.ActivateAsync();

            Task wrapped = null;
            ResultHandle wrapperDone = new ResultHandle();
            ResultHandle wrappedDone = new ResultHandle();
            ResultHandle continuationDone = new ResultHandle();
            Task<Task> wrapper = new Task<Task>(() =>
            {
                ...
                Task t1 = grain.Test1();
                wrapped = t1.ContinueWith(
                    t => { wrappedDone.Done = true; },
                    CancellationToken.None,
                    TaskContinuationOptions.ExecuteSynchronously,
                    TaskScheduler.Default);

                wrapperDone.Done = true;
                return AsyncCompletion.FromTask(wrapped).ContinueWith(() =>
                {
                    continuationDone.Done = true;
                }).AsTask();
            });
            wrapper.Start(scheduler);
            await wrapper;
            Assert.IsTrue(wrapperDone.Done, "Wrapper Task is done");
            Assert.IsTrue(wrapper.IsCompleted, "Wrapper Task completed");

            AsyncCompletion ac2 = AsyncCompletion.FromTask(wrapper)
                .ContinueWith(() => AsyncCompletion.Done);

            await ac2.AsTask();

            await wrapped;
            Assert.IsTrue(wrappedDone.Done, "Wrapped Task is done");
            Assert.IsTrue(wrapped.IsCompleted, "Wrapped Task completed");

            await wrapper.Unwrap();
            bool done = continuationDone.WaitForFinished(TimeSpan.FromMilliseconds(1));
            Assert.IsTrue(done, "AC continuation finished");
            Assert.IsTrue(continuationDone.Done, "AC continuation is done");
        }
        ...
        internal static async Task Run_ActivationSched_Test1(TaskScheduler scheduler, bool bounceToThreadPool)
        {
            ...
            Task<Task> wrapper = new Task<Task>(() =>
            {
                ...
                Action wrappedDoneAction = () => { wrappedDone.Done = true; };
            ...
            wrapper.Start(scheduler);
            await wrapper;
            bool done = wrapperDone.WaitForFinished(TimeSpan.FromMilliseconds(1));
            ...
            await wrapped;
            done = wrappedDone.WaitForFinished(TimeSpan.FromMilliseconds(1));
