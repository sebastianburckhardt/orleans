    ...
    public class Benchmark : IBenchmark
    {
        ...
        private IScenario[] scenarios = new IScenario[] 
        {
            ...
            new SequencedSize(100,180,75,25,0,0,1024),
              // Sequenced Grain. 50 Global / 50 Local
            new SequencedSize(100,180,50,50,0,0,1024),
              // Sequenced Grain. 25 Global / 75 Local
            new SequencedSize(100,180,25,75,0,0,1024),
         /* 3.1.3 1MB*/
            // No replication
            new NoReplicationSize(100, 180,100, 1048576),
            // Sequenced Grain. All Global Reads
            new SequencedSize(100,180,100,0,0,0, 1048576),
            // Sequenced Grain, All Local Reads
            new SequencedSize(100,180,0,100,0,0, 1048576),
            // Sequenced Grain. 75 Global / 25 Local
            new SequencedSize(100,180,75,25,0,0, 1048576),
              // Sequenced Grain. 50 Global / 50 Local
            new SequencedSize(100,180,50,50,0,0, 1048576),
              // Sequenced Grain. 25 Global / 75 Local
            new SequencedSize(100,180,25,75,0,0, 1048576),
        
            /* 3.2 Write-Only Benchmarks */
             /* 3.2.1 128B*/
            // No replication
            new NoReplicationSize(100, 180,0,128),
            // Sequenced Grain. All Global writes
            new SequencedSize(100,180,0,0,100,0,128),
                        new SequencedSize(100,30,0,0,100,0,128),

            // Sequenced Grain, All Local writes
            new SequencedSize(100,180,0,0,0,100,128),
            // Sequenced Grain. 75 Global / 25 Local
            new SequencedSize(100,180,0,0,75,25,128),
              // Sequenced Grain. 50 Global / 50 Local
            new SequencedSize(100,180,0,0,50,50,128),
              // Sequenced Grain. 25 Global / 75 Local
            new SequencedSize(100,180,0,0,25,75,128),
             /* 3.2.2 1KB*/
            new NoReplicationSize(100, 180,0,1024),
            // Sequenced Grain. All Global writes
            new SequencedSize(100,180,0,0,100,0,1024),
            // Sequenced Grain, All Local writes
            new SequencedSize(100,180,0,0,0,100,1024),
            // Sequenced Grain. 75 Global / 25 Local
            new SequencedSize(100,180,0,0,75,25,1024),
              // Sequenced Grain. 50 Global / 50 Local
            new SequencedSize(100,180,0,0,50,50,1024),
              // Sequenced Grain. 25 Global / 75 Local
            new SequencedSize(100,180,0,0,25,75,1024),
             /* 3.2.3 1MB*/
            new NoReplicationSize(100, 180,0, 1048576),
            // Sequenced Grain. All Global writes
            new SequencedSize(100,180,0,0,100,0, 1048576),
            // Sequenced Grain, All Local writes
            new SequencedSize(100,180,0,0,0,100, 1048576),
            // Sequenced Grain. 75 Global / 25 Local
            new SequencedSize(100,180,0,0,75,25, 1048576),
              // Sequenced Grain. 50 Global / 50 Local
            new SequencedSize(100,180,0,0,50,50, 1048576),
              // Sequenced Grain. 25 Global / 75 Local
            new SequencedSize(100,180,0,0,25,75, 1048576),

            /* 3.3 Read-Write Benchmarks */
            /* 3.3.4 Read mostly (ratio rw: 90/10) */

              // no replication
              new NoReplicationSize(100, 180,90,128),
              // Sequenced Grain. All Global ops
             new SequencedSize(100,180,90,0,10,0,128),
              // Sequenced Grain. All Local ops
             new SequencedSize(100,180,0,90,0,10,128),
              // Sequenced Grain. 50/50 Global/Local
             new SequencedSize(100,180,45,45,5,5,128),
             // Sequenced Grain Local Reads, Global Writes
             new SequencedSize(100,180,0,90,10,0,128),
             // Sequenced Grain Global Reads, Local Writes
             new SequencedSize(100,180,90,0,0,10,128),
             // Sequenced Grain 50/50 Local/Global Reads, Local Writes
             new SequencedSize(100,180,45,45,0,10,128),
              // Sequenced Grain 50/50 Local/Global Reads, Global Writes
             new SequencedSize(100,180,45,45,10,0,128),
             // Sequenced Grain Global Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,90,0,5,5,128),
             // Sequenced Grain Local Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,0,90,5,5,128),

                 // no replication
              new NoReplicationSize(100, 180,90, 1024),
              // Sequenced Grain. All Global ops
             new SequencedSize(100,180,90,0,10,0, 1024),
              // Sequenced Grain. All Local ops
             new SequencedSize(100,180,0,90,0,10, 1024),
              // Sequenced Grain. 50/50 Global/Local
             new SequencedSize(100,180,45,45,5,5, 1024),
             // Sequenced Grain Local Reads, Global Writes
             new SequencedSize(100,180,0,90,10,0, 1024),
             // Sequenced Grain Global Reads, Local Writes
             new SequencedSize(100,180,90,0,0,10, 1024),
             // Sequenced Grain 50/50 Local/Global Reads, Local Writes
             new SequencedSize(100,180,45,45,0,10, 1024),
              // Sequenced Grain 50/50 Local/Global Reads, Global Writes
             new SequencedSize(100,180,45,45,10,0, 1024),
             // Sequenced Grain Global Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,90,0,5,5, 1024),
             // Sequenced Grain Local Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,0,90,5,5, 1024),

                 // no replication
              new NoReplicationSize(100, 180,90, 1048576),
              // Sequenced Grain. All Global ops
             new SequencedSize(100,180,90,0,10,0, 1048576),
              // Sequenced Grain. All Local ops
             new SequencedSize(100,180,0,90,0,10, 1048576),
              // Sequenced Grain. 50/50 Global/Local
             new SequencedSize(100,180,45,45,5,5, 1048576),
             // Sequenced Grain Local Reads, Global Writes
             new SequencedSize(100,180,0,90,10,0, 1048576),
             // Sequenced Grain Global Reads, Local Writes
             new SequencedSize(100,180,90,0,0,10, 1048576),
             // Sequenced Grain 50/50 Local/Global Reads, Local Writes
             new SequencedSize(100,180,45,45,0,10, 1048576),
              // Sequenced Grain 50/50 Local/Global Reads, Global Writes
             new SequencedSize(100,180,45,45,10,0, 1048576),
             // Sequenced Grain Global Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,90,0,5,5, 1048576),
             // Sequenced Grain Local Reads, 50/50 Local/Global Writes
             new SequencedSize(100,180,0,90,5,5, 1048576),

            /* 3.3.5 Write heavy (ratio rw: 70/30) */
            new NoReplicationSize(100, 180,70, 128),
                // Sequenced Grain. All Global ops
             new SequencedSize(100,180,70,0,30,0, 128),
              // Sequenced Grain. All Local ops
             new SequencedSize(100,180,0,70,0,30, 128),
              // Sequenced Grain. 50/50 Global/Local
             new SequencedSize(100,180,35,35,15,15, 128),
             // Sequenced Grain Local Reads, Global Writes
             new SequencedSize(100,180,0,70,30,0, 128),
             // Sequenced Grain Global Reads, Local Writes
             new SequencedSize(100,180,70,0,0,30, 128),
             // Sequenced Grain 50/50 Local/Global Reads, Local Writes
             new SequencedSize(100,180,35,35,0,30, 128),
              // Sequenced Grain 50/50 Local/Global Reads, Global Writes
             new SequencedSize(100,180,35,35,30,0, 128),
             // Sequenced Grain Global Reads, 50/50 Local/Global Writes
        ...
        public IRequest ParseRequest(string verb, IEnumerable<string> urlpath, NameValueCollection arguments, string body)
        {
            ...
            if (verb == "GET" && string.Join("/", urlpath) == "size")
            {
                ...
                if (int.Parse(arguments["rep"]) == 0)
                {
                    ...
                    {
                        // WRITE type
                        GeoOrleans.Runtime.Common.Util.Assert(false, "Should be of POST type");
                    }
            ...
            else if (verb == "POST" && string.Join("/", urlpath) == "size")
            {
                ...
                if (int.Parse(arguments["rep"]) == 0)
                {
                    ...
                    {
                        GeoOrleans.Runtime.Common.Util.Assert(false, "Incorrect message type");
                    }
