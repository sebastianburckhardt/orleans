...
namespace UnitTests
{
    ...
    public class DeployTest
    {
        ...
        public void InitializeTest()
        {
            ...
            Collection<PSObject> results = this.RunPowerShellCommand("Get-ExecutionPolicy");
            oldExecutionPolicy = results[0].ToString();
            if (string.Compare(oldExecutionPolicy, "restricted", true) == 0)
            {
                if (RunningAsAdmin())
                {
                    this.RunPowerShellCommand("Set-ExecutionPolicy RemoteSigned -Force -Scope LocalMachine");
                }
                else
                {
        ...
        public void CleanupTest()
        {
            ...
            if (RunningAsAdmin())
            {
                this.RunPowerShellCommand("Set-ExecutionPolicy", oldExecutionPolicy, "-Force", "-Scope LocalMachine");
            }
        }
        ...
        public void CleanOrleans_OrleansAndCacheDirsExist_OrleansFilesDeleted()
        {
            ...
            if (!Directory.Exists(factoryCacheFolder))
            {
                ...
                // TODO: Maybe copy some files here to confirm that the delete actually does something.
            }
            ...
            Assert.AreEqual<int>(0, cacheFileCountAfterScript, "{0} files were not deleted from the cache folder ({1}).", cacheFileCountAfterScript, factoryCacheFolder);
        }
        ...
        public void IsProcessRunning_ProcessNotRunning_ReportFalse()
        {
            ...
            if (results.Count != 0)
            {
                ...
                Assert.Fail("IsProcessRunning.ps1 script returned information when it should have return nothing: {0}", resultsBuilder.ToString());
            }
        }
