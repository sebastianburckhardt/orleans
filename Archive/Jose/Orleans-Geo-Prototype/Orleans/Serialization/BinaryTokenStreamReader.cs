...
namespace Orleans.Serialization
{
    ...
    {
        private readonly List<ArraySegment<byte>> buffers;
        ...
        public BinaryTokenStreamReader(List<ArraySegment<byte>> buffs)
        {
            buffers = buffs;
        ...
        private ArraySegment<byte> CheckLength(int n, out bool safeToUse)
        {
            ...
            if ((CurrentPosition + n > totalLength) || (currentSegmentIndex >= buffers.Count))
            {
                throw new SerializationException("Attempt to read past the end of the input stream");
            }
        ...
        public string ReadString()
        {
            ...
            var s = Encoding.UTF8.GetString(buff.Array, buff.Offset, n);
        ...
        public SiloAddress ReadSiloAddress()
        {
            ...
            var gen = ReadInt();
            var cluster = ReadInt();
            return SiloAddress.New(ep, gen, cluster);
        }
        ...
        {
            int keyExtLen = ReadByte();
            var bytes = ReadBytes(UniqueKey.CalculateByteArrayLengthForBinaryTokenStreamReader(keyExtLen));
            return GrainId.GetGrainId(bytes, keyExtLen);
        }
        ...
        internal ActivationId ReadActivationId()
        {
            ...
            {
                var bytes = ReadBytes(UniqueKey.CalculateByteArrayLengthForBinaryTokenStreamReader(keyExtLen));
                return ActivationId.GetActivationId(bytes);
            }
            else
                throw new InvalidDataException("An activation ID should always be encoded without a key extension.");
        }
        ...
        internal ActivationAddress ReadActivationAddress()
        {
            ...
            if (silo == SiloAddress.Zero)
            {
                silo = null;
            }
            if (act == ActivationId.Zero)
            {
                act = null;
            }
        ...
        internal bool TryReadSimpleType(out object result, out SerializationTokenType token)
        {
            ...
            switch (token)
            {
                ...
                case SerializationTokenType.CorrelationId:
                    result = new CorrelationId(ReadBytes(CorrelationId.SizeBytes));
                    break;
                default:
                    result = null;
                    return false;
