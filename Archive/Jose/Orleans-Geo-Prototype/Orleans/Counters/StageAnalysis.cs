    ...
    */
    ...
    internal class StageAnalysis
    {
        ...
        public StageAnalysis()
        {
            ...
            {
                StringValueStatistic.FindOrCreate(StatNames.STAT_STAGE_ANALYSIS, () =>
                {
                    return StageAnalysisInfo();
                });
            }
        }
        ...
        public void AddTracking(ThreadTrackingStatistic tts)
        {
            lock (stageGroups)
            {
                ...
                List<ThreadTrackingStatistic> stageGroup = null;
        ...
        private static HashSet<string> waitingStages = new HashSet<string>()
        {
        ...
        public string StageAnalysisInfo()
        {
            lock (stageGroups)
            {
                ...
                Dictionary<string, double> waitTimes = new Dictionary<string, double>(); // Wait time per request for each stage
                foreach (var stage in wallClockPerRequest.Keys)
                {
                    if (waitingStages.Contains(stage))
                    {
                        waitTimes.Add(stage, Math.Max(wallClockPerRequest[stage] - avgRatio * cpuPerRequest[stage] - cpuPerRequest[stage], 0));
                    }
                    else
                    {
                        waitTimes.Add(stage, 0);
                    }
                }
                ...
                Dictionary<string, double> t = new Dictionary<string, double>(); // Thread allocation suggestion for each stage
                foreach (var stage in wallClockPerRequest.Keys)
                {
                    // cores is p
                    ...
                    // waitTimesis w
                    t.Add(stage, cores * numberOfRequests[stage] * (cpuPerRequest[stage] * (1 + stableReadyTimeProportion) + waitTimes[stage]) / cpuSum);
                }
                ...
                foreach (var stage in stages.Intersect(wallClockPerRequest.Keys))
                {
                    ...
                    sb.AppendLine("  Suggested thread allocation:             " + Math.Ceiling(t[stage]) + " threads  (rounded up from " + t[stage].ToString("F3") + ")");
                }

                Dictionary<string, double> cpuBreakdown = new Dictionary<string, double>();
                foreach (var stage in cpuPerRequest.Keys)
                {
                    double val = (numberOfRequests[stage] * cpuPerRequest[stage]) / (cores * elapsedWallClock);
                    cpuBreakdown.Add(stage, val);
                }
        ...
        private double GetWallClockPerStagePerRequest(string stageName)
        {
            ...
            foreach (var statistics in stageGroups[stageName])
            {
                ...
                else
                {
                    ...
                    TimeSpan takeCPUCycles = statistics.executingCPUCycleTime.Elapsed - statistics.processingCPUCycleTime.Elapsed;
