...
namespace Orleans.Serialization
{
    internal static class BuiltInTypes
    {
        ...
        internal static void SerializeKeyValuePair<K, V>(object original, BinaryTokenStreamWriter stream, Type expected)
        {
            var pair = (KeyValuePair<K, V>)original;
            SerializationManager.SerializeInner(pair.Key, stream, typeof(K));
            SerializationManager.SerializeInner(pair.Value, stream, typeof(V));
        }
        ...
        internal static object CopyKeyValuePair<K, V>(object original)
        {
            var pair = (KeyValuePair<K, V>)original;
            if (typeof(K).IsOrleansShallowCopyable() && typeof(V).IsOrleansShallowCopyable())
            {
            ...
            var result = new KeyValuePair<K, V>((K)SerializationManager.DeepCopyInner(pair.Key), (V)SerializationManager.DeepCopyInner(pair.Value));
            SerializationContext.Current.RecordObject(original, result);
            return result;
        }
        ...
        //    Type t = original.GetType();
        //    var generics = t.GetGenericArguments();
        //    var concretes = RegisterConcreteMethods(t, "SerializeFSharpList", "DeserializeFSharpList", "DeepCopyFSharpList", generics);
        ...
        //{
        //    var generics = expected.GetGenericArguments();
        //    var concretes = RegisterConcreteMethods(expected, "SerializeFSharpList", "DeserializeFSharpList", "DeepCopyFSharpList", generics);
        ...
        internal static object DeserializeGrainId(Type expected, BinaryTokenStreamReader stream)
        {
            ...
            //var bytes = stream.ReadBytes(2 * sizeof(long));
            //return GrainId.GetGrainId(bytes);
        }
        ...
        {
            var bytes = stream.ReadBytes(CorrelationId.SizeBytes);
        ...
        #region OrleansResponse

        internal static void SerializeOrleansResponse(object obj, BinaryTokenStreamWriter stream, Type expected)
        {
            var resp = (OrleansResponse)obj;

            if (resp.ExceptionFlag)
            {
                SerializationManager.SerializeInner(resp.Exception, stream, null);
            }
            else
            {
                SerializationManager.SerializeInner(resp.Data, stream, null);
            }
        }
        ...
        internal static object DeserializeOrleansResponse(Type expected, BinaryTokenStreamReader stream)
        {
            ...
            return new OrleansResponse(obj);
        }

        internal static object CopyOrleansResponse(object original)
        {
            var resp = (OrleansResponse)original;
            ...
            var result = new OrleansResponse(SerializationManager.DeepCopyInner(resp.Data));
            SerializationContext.Current.RecordObject(original, result);
            return result;
        }
