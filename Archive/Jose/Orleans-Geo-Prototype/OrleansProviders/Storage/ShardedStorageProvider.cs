...
namespace Orleans.Storage
{
    ...
    public class ShardedStorageProvider : IStorageProvider
    {
        ...
        private IStorageProvider[] _storageProviders;
        ...
        {
            return StorageProviderUtility.PositiveHash(grainReference, _storageProviders.Length);
        }
        ...
        public Task Init(string name, IProviderRuntime providerRuntime, IProviderConfiguration config)
        {
            ...
            Log = providerRuntime.GetLogger("Storage.ShardedStorageProvider." + _id, Logger.LoggerType.Application);
            ...
            _storageProviders = providers.ToArray();
        ...
        public Task Close()
        {
            List<Task> closePromises = new List<Task>();
            foreach (var provider in _storageProviders)
            {
                Task close = provider.Close();
                closePromises.Add(close);
            }
            return Task.WhenAll(closePromises);
        }
        ...
        public Task ReadStateAsync(string grainType, GrainReference grainReference, IGrainState grainState)
        {
            ...
            IStorageProvider provider = _storageProviders[num];
        ...
        private int FindStorageShard(string grainType, GrainReference grainReference)
        {
            ...
            if (num < 0 || num >= _storageProviders.Length)
            {
                string msg = String.Format("Hash function returned out of bounds value {0}. This is an error.", num);
