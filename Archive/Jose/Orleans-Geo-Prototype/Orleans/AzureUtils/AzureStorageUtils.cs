...
namespace Orleans.AzureUtils
{
    ...
    public static class AzureStorageUtils
    {
        ...
        public static bool TableStorageDataNotFound(Exception exc)
        {
            ...
            DataServiceClientException dsce = exc as DataServiceClientException;
            if (dsce == null)
            {
                DataServiceRequestException dsre = exc as DataServiceRequestException;
                if (dsre != null)
                {
            ...
            if (StorageErrorCodeStrings.ResourceNotFound.Equals(restErrorCode))
            {
                return true;
            }
            return false;
        ...
        public static string ExtractRestErrorCode(Exception exc)
        {
            ...
            if (exc is DataServiceClientException)
            {
                ...
                {
                    var log = Logger.GetLogger("AzureStorageUtils", Logger.LoggerType.Runtime);
        ...
        internal static void ValidateTableName(string tableName)
        {
            ...
            if (!tableName.All(c => Char.IsLetterOrDigit(c)))
            {
        ...
        internal static string PrintCloudQueueMessage(CloudQueueMessage message)
        {
            return String.Format("CloudQueueMessage: Id = {0}, NextVisibleTime = {1}, DequeueCount = {2}, PopReceipt = {3}, Content = {4}",
                    message.Id,
                    message.NextVisibleTime.HasValue ? Logger.PrintDate(message.NextVisibleTime.Value) : "",
        ...
        internal static bool AnalyzeReadException(Exception exc, int iteration, string tableName, Logger logger)
        {
            ...
            WebException we = exc as WebException;
            if (we != null)
            {
