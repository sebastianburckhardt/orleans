...
namespace UnitTests.Streaming.Reliability
{
    ...
    public class StreamReliabilityTests : UnitTestBase
    {
        ...
        //private readonly int NumItems = 100;
        ...
        private static readonly Options siloRunOptions = new Options
        {
            // StartSecondary = false, // Temp HACK for testing
        ...
        private static readonly ClientOptions clientRunOptions = new ClientOptions
        {
        ...
        public void TestCleanup()
        {
            ...
            {
                var queues = (new AzureQueueStreamQueueMapper()).GetAllQueues().ToList();
                logger.Info("TestCleanup - DeleteAllAzureQueues Count={0}", queues.Count);
                DeleteAllAzureQueues(queues, UnitTestBase.DeploymentId, StandaloneAzureQueueTests.DataConnectionString, logger);
            }
#if DELETE_AFTER_TEST
        ...
        [TestMethod, TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability")]
        ...
        [TestMethod, TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability"), TestCategory("Azure")]
        ...
        // TODO: [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability")]
        ...
        // TODO: [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability"), TestCategory("Azure")]
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability")]
        ...
        [TestMethod, TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability"), TestCategory("Azure"), TestCategory("Queue")]
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("Streaming"), TestCategory("Reliability"), TestCategory("Azure"), TestCategory("Queue")]
        ...
        private async Task Test_SiloJoins(string testName, string streamProviderName)
        {
            ...
            // Add new silo
            SiloHandle newSilo = base.StartAdditionalOrleans();
            base.WaitForLivenessToStabilize(true);
        ...
        private void RestartAllSilos()
        {
            ...
            Initialize(siloRunOptions, clientRunOptions);
        ...
        private void StopSilo(SiloHandle silo, bool kill, bool restart)
        {
            ...
            if (restart)
            {
                ...
                SiloHandle newSilo = isPrimary ? Primary : Secondary;
        ...
        {
            SiloAddress primSilo = Primary.Silo.SiloAddress;
            SiloAddress secSilo = Secondary != null ? Secondary.Silo.SiloAddress : null;
            logger.Info("\n\n**START********************** {0} ********************************* \n\n"
                        + "Running with initial silos Primary={1} Secondary={2} StreamId={3} StreamType={4} \n\n",
                testName, primSilo, secSilo, _streamId, _streamProviderName);
        }
...
#if USE_GENERICS
        private IStreamReliabilityTestGrain<int> GetGrain(long grainId)
#else
        private IStreamReliabilityTestGrain GetGrain(long grainId)
#endif
        {
#if USE_GENERICS
            return StreamReliabilityTestGrainFactory<int>.GetGrain(grainId);
#else
            return StreamReliabilityTestGrainFactory.GetGrain(grainId);
#endif
        }
        ...
        private async Task CheckConsumerProducerStatus(string when, long producerGrainId, long consumerGrainId, bool expectIsProducer, bool expectIsConsumer)
        {
            await CheckConsumerProducerStatus(when, producerGrainId, consumerGrainId, 
                expectIsProducer ? 1 : 0, 
                expectIsConsumer ? 1 : 0);
        }
        private async Task CheckConsumerProducerStatus(string when, long producerGrainId, long consumerGrainId, int expectedNumProducers, int expectedNumConsumers)
        {
            var producerGrain = GetGrain(producerGrainId);
            var consumerGrain = GetGrain(consumerGrainId);
        ...
        private static async Task<bool> CheckGrainCounts()
        {
#if USE_GENERICS
            ...
            Console.WriteLine("Found grains " + Utils.IEnumerableToString(grainStats));
            ...
            if (sameSilo)
            {
                Assert_AreEqual(2, grainLocs[0].ActivationCount, "Num grains on same Silo " + grainLocs[0].SiloAddress);
            }
            else
            {
                Assert_AreEqual(1, grainLocs[0].ActivationCount, "Num grains on Silo " + grainLocs[0].SiloAddress);
                Assert_AreEqual(1, grainLocs[1].ActivationCount, "Num grains on Silo " + grainLocs[1].SiloAddress);
            }
            return sameSilo;
        }

#if USE_GENERICS
        private async Task CheckReceivedCounts<T>(string when, IStreamReliabilityTestGrain<T> consumerGrain, int expectedReceivedCount, int expectedErrorsCount)
#else
        private async Task CheckReceivedCounts(string when, IStreamReliabilityTestGrain consumerGrain, int expectedReceivedCount, int expectedErrorsCount)
#endif
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                receivedCount = await consumerGrain.ReceivedCount;
            ...
            Assert_AreEqual(expectedReceivedCount, receivedCount,
                "ReceivedCount for stream {0} for grain {1} {2}", _streamId, pk, when);

            int errorsCount = await consumerGrain.ErrorsCount;
            Assert_AreEqual(expectedErrorsCount, errorsCount, "ErrorsCount for stream {0} for grain {1} {2}", _streamId, pk, when);
        }
#if USE_GENERICS
        private async Task CheckConsumerCounts<T>(string when, IStreamReliabilityTestGrain<T> consumerGrain, int expectedConsumerCount)
#else
        private async Task CheckConsumerCounts(string when, IStreamReliabilityTestGrain consumerGrain, int expectedConsumerCount)
#endif
        {
            int consumerCount = await consumerGrain.ConsumerCount;
            Assert_AreEqual(expectedConsumerCount, consumerCount, "ConsumerCount for stream {0} {1}", _streamId, when);
        }

        private async Task CheckPubSubCounts(string when, int expectedPublisherCount, int expectedConsumerCount)
        {
            IPubSubRendezvousGrain rvGrain = PubSubRendezvousGrainFactory.GetGrain(_streamId.AsGuid);

            int consumerCount = await rvGrain.ConsumerCount(_streamId);
            PubSubSubscriptionState[] consumers = await rvGrain.DiagGetConsumers(_streamId);

            Assert_AreEqual(expectedConsumerCount, consumerCount, "{0} - ConsumerCount for stream {1}. Consumers = {2}", 
                when, _streamId, Utils.IEnumerableToString(consumers));

            int publisherCount = await rvGrain.ProducerCount(_streamId);
            
            Assert_AreEqual(expectedPublisherCount, publisherCount, "{0} - PublisherCount for stream {1}", 
                when, _streamId);
        }

        private static void Assert_AreEqual(int expected, int actual, string msg, params object[] args)
        {
            // expected == -1 means don't care / don't assert check value.
            string prefix = expected == -1 ? "Ok" : actual == expected ? "True" : "FALSE";
