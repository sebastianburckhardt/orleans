...
namespace UnitTests.General
{
    ...
    public class RequestContextTests_Local
    {
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public async Task RequestContext_MultiThreads_ExportToMessage()
        {
            ...
            for (int i = 0; i < NumLoops; i++)
            {
                ...
                {
                    flag.Wait();
                    RequestContext.ExportToMessage(msg);
                });
                flag.Set();
                Thread.Sleep(1);
        ...
        public void RequestContext_ActivityId_ExportToMessage()
        {
            ...
            Message msg = new Message();
            RequestContext.ExportToMessage(msg);
            msg.GetApplicationHeaders(headers);
            Assert.IsFalse(headers.ContainsKey(RequestContext.E2ETracing_ActivityId_Header), "ActivityId should not be be present " + headers.ToStrings(separator: ","));
            TestCleanup();
            ...
            RequestContext.ExportToMessage(msg);
            msg.GetApplicationHeaders(headers);
            Assert.IsTrue(headers.ContainsKey(RequestContext.E2ETracing_ActivityId_Header), "ActivityId #1 should be present " + headers.ToStrings(separator: ","));
            object result = msg.GetApplicationHeader(RequestContext.E2ETracing_ActivityId_Header);
            ...
            RequestContext.ExportToMessage(msg);
            msg.GetApplicationHeaders(headers);
            Assert.IsFalse(headers.ContainsKey(RequestContext.E2ETracing_ActivityId_Header), "Null ActivityId should not be present " + headers.ToStrings(separator: ","));
            TestCleanup();
            ...
            RequestContext.ExportToMessage(msg);
            msg.GetApplicationHeaders(headers);
            Assert.IsTrue(headers.ContainsKey(RequestContext.E2ETracing_ActivityId_Header), "ActivityId #2 should be present " + headers.ToStrings(separator: ","));
            result = msg.GetApplicationHeader(RequestContext.E2ETracing_ActivityId_Header);
        ...
        public void RequestContext_ActivityId_ExportImport()
        {
            ...
            RequestContext.ExportToMessage(msg);
            RequestContext.Clear();
            RequestContext.ImportFromMessage(msg);
            var actId = RequestContext.Get(RequestContext.E2ETracing_ActivityId_Header);
            ...
            RequestContext.Clear();
            RequestContext.ImportFromMessage(msg);
            actId = RequestContext.Get(RequestContext.E2ETracing_ActivityId_Header);
        ...
        // TODO: [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        [TestMethod, TestCategory("Failures"), TestCategory("RequestContext")]
        ...
        //[TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        //public async Task RequestContext_CrossThread_Dictionary()
        //{
        //    const int NumLoops = 1000;
        ...
        //    var dict = new Dictionary<string, string>();
        //    dict[name1] = data1;
        //    RequestContext.Set(name1, dict);

        //    var result1 = (Dictionary<string, string>)RequestContext.Get(name1);
        //    Assert.AreEqual(data1, result1[name1], "RC.GetData-Main");
        ...
        //        promises[i] = Task.Run(async () =>
        //        {
        //            var dict1 = (Dictionary<string, string>)CallContext.LogicalGetData(name1);
        //            if (dict1 == null) dict1 = new Dictionary<string, string>();
        //            dict1[name1] = str;
        //            CallContext.LogicalSetData(name1, dict1);

        //            var dict2 = (Dictionary<string, string>)RequestContext.Get(name1);
        //            Assert.AreSame(dict, dict2, "Same object RC.GetData-Task.Run-Get" + str);
        //            Assert.AreEqual(str, dict2[name1], "RC.GetData-Task.Run-Get-" + str);

        //            var dict3 = new Dictionary<string, string>();
        //            dict3[name1] = data1;
        //            //RequestContext.Set(name1, dict3);

        //            await Task.Delay(5);
        //            var result3 = (Dictionary<string, string>)RequestContext.Get(name1);
        //            Assert.AreSame(dict3, result3, "Same object RC.GetData-Task.Run-Set-1-" + str);
        //            Assert.AreEqual(str, result3[name1], "RC.GetData-Task.Run-Set-" + str);

        //            flag.Wait();
        //            await Task.Delay(5);
        //            result3 = (Dictionary<string, string>)RequestContext.Get(name1);
        //            Assert.AreSame(dict3, result3, "Same object RC.GetData-Task.Run-Set-2-" + str);
        //            Assert.AreEqual(str, result3[name1], "RC.GetData-Task.Run-Set-" + str);
        //        });
        //    }
        //    for (int i = 0; i < NumLoops; i++)
        //    {
        //        flag.Set(); // Trigger Task
        //    }
        //    await Task.WhenAll(promises);
        //    result1 = (Dictionary<string, string>)RequestContext.Get(name1);
        //    Assert.AreEqual(data1, result1[name1], "RC.GetData-Main-Final");
        //}
    }

    [TestClass]
    public class RequestContextTests_Silo : UnitTestBase
    {
        private static bool oldPropagateActivityId;
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_AC_Test1()
        {
        ...
        public async Task RequestContext_Task_Test1()
        {
            ...
            var grain = RequestContextTaskGrainFactory.GetGrain(id);
        ...
        {
            var grain = RequestContextTaskGrainFactory.GetGrain(1);
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_RC_Set_E2E()
        {
            Guid activityId = Guid.NewGuid();
            Guid activityId2 = Guid.NewGuid();
            Guid nullActivityId = Guid.Empty;

            IRequestContextTestGrain grain = RequestContextTestGrainFactory.GetGrain(GetRandomGrainId());

            RequestContext.Set(RequestContext.E2ETracing_ActivityId_Header, activityId);
            Guid result = grain.E2EActivityId().Result;
            Assert.AreEqual(activityId, result, "E2E ActivityId not propagated correctly");
            RequestContext.Clear();

            RequestContext.Set(RequestContext.E2ETracing_ActivityId_Header, nullActivityId);
            result = grain.E2EActivityId().Result;
            Assert.AreEqual(nullActivityId, result, "Null ActivityId propagated E2E incorrectly");
            RequestContext.Clear();

            RequestContext.Set(RequestContext.E2ETracing_ActivityId_Header, activityId2);
            result = grain.E2EActivityId().Result;
            Assert.AreEqual(activityId2, result, "E2E ActivityId 2 not propagated correctly");
            RequestContext.Clear();
        }
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_CM_E2E()
        {
            Guid activityId = Guid.NewGuid();
            Guid activityId2 = Guid.NewGuid();
            Guid nullActivityId = Guid.Empty;
            ...
            Trace.CorrelationManager.ActivityId = activityId;
            Assert.IsNull(RequestContext.Get(RequestContext.E2ETracing_ActivityId_Header), "No ActivityId context should be set");
            Guid result = grain.E2EActivityId().Result;
            Assert.AreEqual(activityId, result, "E2E ActivityId not propagated correctly");
            RequestContext.Clear();
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_CM_E2E_ViaProxy()
        {
            Guid activityId = Guid.NewGuid();
            Guid activityId2 = Guid.NewGuid();
            Guid nullActivityId = Guid.Empty;

            IRequestContextProxyGrain grain = RequestContextProxyGrainFactory.GetGrain(GetRandomGrainId());
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_RC_None_E2E()
        {
            Guid nullActivityId = Guid.Empty;
            ...
            Assert.AreEqual(null, RequestContext.Get(RequestContext.E2ETracing_ActivityId_Header), "No ActivityId context should be set");
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_CM_None_E2E()
        {
            Guid nullActivityId = Guid.Empty;
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_DynamicChange_Client()
        {
            Guid activityId = Guid.NewGuid();
            Guid activityId2 = Guid.NewGuid();
        ...
        [TestMethod, TestCategory("Failures"), TestCategory("RequestContext")]
        // TODO: [TestMethod, TestCategory("BVT"), TestCategory("Nightly"), TestCategory("RequestContext")]
        public void RequestContext_ActivityId_DynamicChange_Server()
        {
            Guid activityId = Guid.NewGuid();
            Guid activityId2 = Guid.NewGuid();
        ...
        public async Task ClientInvokeCallback_GrainObserver()
        {
            TestClientInvokeCallback callback = new TestClientInvokeCallback(Guid.Empty);
            GrainClient.ClientInvokeCallback = callback.OnInvoke;
            ObserverTests.SimpleGrainObserver observer = new ObserverTests.SimpleGrainObserver(null, null);
            ...
            ISimpleGrainObserver reference = await SimpleGrainObserverFactory.CreateObjectReference(observer);
