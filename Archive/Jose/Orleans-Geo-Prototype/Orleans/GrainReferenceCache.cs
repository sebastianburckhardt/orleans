...
namespace Orleans
{
        ...
        where TValue : IAddressable
    {
        ...
        private Dictionary<TKey, TimestampedValue> cache;

        private readonly FetchValueDelegate fetcher;
        private readonly CastDelegate cast;
                ...
                while (cache.Count >= maximumCount)
                {
                    ...
                    KeyValuePair<TKey, TimestampedValue> entryToFree =
                        cache.FirstOrDefault(
                            (KeyValuePair<TKey, TimestampedValue> kvp) => kvp.Value.generation == generationToDelete);
