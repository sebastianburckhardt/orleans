...
namespace UnitTests.TimerTests
{
    ...
    public class TimerOrleansTest : UnitTestBase
    {
        ...
        public void TimerOrleansTest_Migration()
        {
            ...
            //IOrleansManagementGrain mgmtGrain = Orleans.Silo.SystemManagement;
        ...
        public async Task AsyncTimerTest_GrainCall()
        {
            ...
            try
            {
                ...
                int tickCount = await grain.TickCount;
                Assert.AreEqual(1, tickCount, "Should be {0} timer callback", tickCount);

                Exception err = await grain.Exception;
    ...
    public class TimerGrainReferenceProxy// : UnitTestGrains.ITimerGrain
    {
        ...
        public TimerGrainReferenceProxy(/*StringGrainKey grainId, */bool persist)
        {
            persistant = persist;
            if (persistant)
            {
                persistantGrain = TimerPersistantGrainFactory.GetGrain(UnitTestBase.GetRandomGrainId()); //.GetReference(grainId);
            }
            else
            {
                grain = TimerGrainFactory.GetGrain(UnitTestBase.GetRandomGrainId()); //.GetReference(grainId);
            }
        }
