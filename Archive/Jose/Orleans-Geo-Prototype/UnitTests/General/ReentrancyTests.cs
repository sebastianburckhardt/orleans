    ...
    public class ReentrancyTests : UnitTestBase
    {
        ...
        public void NonReentrantGrain()
        {
            ...
            catch (Exception exc)
            {
                ...
                if (baseExc.GetType().Equals(typeof(OrleansDeadlockException)))
                {
                    deadlock = true;
                }
                else
        ...
        {
            var grainTypeManager = new GrainTypeManager(true);
            grainTypeManager.Start();

            GrainBase grain;
            GrainTypeData data;
            
            grain = new ReentrantGrain();
            grainTypeManager.TryGetData(TypeUtils.GetFullName(grain.GetType()), out data);
            Assert.IsTrue(data.IsReentrant, "IsReentrant " + TypeUtils.GetFullName(grain.GetType()));

            grain = new NonRentrantGrain();
            grainTypeManager.TryGetData(TypeUtils.GetFullName(grain.GetType()), out data);
            Assert.IsFalse(data.IsReentrant, "IsReentrant " + TypeUtils.GetFullName(grain.GetType()));

            grain = new UnorderedNonRentrantGrain();
            grainTypeManager.TryGetData(TypeUtils.GetFullName(grain.GetType()), out data);
            Assert.IsFalse(data.IsReentrant, "IsReentrant " + TypeUtils.GetFullName(grain.GetType()));
        }
