...
namespace Orleans.Runtime.Scheduler
{
    ...
    {
        private static readonly Logger logger = Logger.GetLogger("InvokeWorkItem", Logger.LoggerType.Runtime);
        ...
        public InvokeWorkItem(ActivationData activation, Message message, ISchedulingContext context)
        {
            ...
            {
                string str = String.Format("Creating InvokeWorkItem with bad activation: {0}. Message: {1}", activation, message);
        ...
        public override void Execute()
        {
            try
            {
                ...
                Task task = InsideGrainClient.Current.Invoke(grain, activation, message);
                task.ContinueWith(t =>
                {
                    // Note: This runs for all outcomes of resultPromiseTask - both Success or Fault
                    activation.DecrementInFlightCount();
                    InsideGrainClient.Current.Dispatcher.OnActivationCompletedRequest(activation, message);
                }).Ignore();
            }
            catch (Exception exc)
            {
