...
{
    public class TimerGrain : GrainBase, ITimerGrain
    {
        int counter = 0;
        Dictionary<string, IOrleansTimer> allTimers;
        IOrleansTimer defaultTimer;
        ...
        {
            logger = (Logger)this.GetLogger("TimerGrain_" + base._Data.Address.ToString());
            ...
            allTimers = new Dictionary<string, IOrleansTimer>();
        ...
        {
            IOrleansTimer timer = this.RegisterTimer(Tick, timerName, TimeSpan.Zero, period);
        ...
        {
            IOrleansTimer timer = allTimers[timerName];
            timer.Dispose();
            return TaskDone.Done;
        }
    ...
    public class TimerCallGrain : GrainBase, ITimerCallGrain
    {
        ...
        public Task<int> TickCount { get { return Task.FromResult(tickCount); } }
        public Task<Exception> Exception { get { return Task.FromResult(tickException); } }

        public override Task ActivateAsync()
        {
            logger = this.GetLogger("TimerCallGrain_" + base._Data.Address);
            context = RuntimeContext.Current.ActivationContext;
        ...
        private async Task ProcessTimerTick(object data)
        {
            ...
            ISimpleGrain grain = SimpleGrainFactory.GetGrain(0, "SimpleGrain");
