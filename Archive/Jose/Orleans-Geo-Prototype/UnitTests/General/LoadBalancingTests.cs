...
namespace UnitTests.General
{
    ...
    public class LoadBalancingTests
    {
        ...
        public void ClientBalancing_CreateMany(int numClients)
        {
            ...
            Console.WriteLine(String.Format("ClientsPerSilo for {0} clients and {1} silos: {2} ", numClients, numSilos, Utils.IEnumerableToString(silos)));
        }
        ...
        private void VirtualBuckets_RunMany(int bucketsPerSilo)
        {
            ...
            max = max / numTests;
            span = span / numTests;
            Console.WriteLine(String.Format("bucketsPerSilo: {0} MAX BUCKET %: {1:F3} , MIN BUCKET %: {2:F3}, SPAN %: {3:F3}", bucketsPerSilo, max / av, min / av, span / av));
            Console.WriteLine("******************************************************");
        }
        ...
        private Tuple<double, double> VirtualBuckets_Analyze(int bucketsPerSilo)
        {
            ...
            if (debug)
            {
                Console.WriteLine("\n\nSUM BUCKETS: " + Utils.IEnumerableToString<int>(array));
            }
