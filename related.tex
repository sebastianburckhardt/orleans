...
\vspace{.05in}\emph{AmbientTalk/R} also introduces reactive programming into the actor model. But its focus is not on services, but on providing reactivity within a mobile ad-hoc network. So-called \emph{ambient behaviours} can be exposed, which allow actors to share behaviours using an intentional description of the behaviour.  

%A decentralized pub/sub mechanism then makes sure proximate actors can connect to each other through these behaviours. No glitch-freedom is guaranteed, since that would not be suited for an ambient network.
