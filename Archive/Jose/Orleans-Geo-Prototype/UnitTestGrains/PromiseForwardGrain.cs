...
namespace UnitTestGrains
{
    ...
    /// </summary>
    public class PromiseForwardGrain : GrainBase<ISimpleGrainState>, IPromiseForwardGrain
    {
        ...
        {
            return GetSimpleGrain().ReadOnlyInterlock(timeout);
        }

        public Task ExclusiveWait(int timeout)
        {
            return GetSimpleGrain().ExclusiveWait(timeout);
        }
        ...
        //public void LongMethod(int waitTime)
        //{
        //    GetSimpleGrain().LongMethod(waitTime).Wait();
        //}
        
        private ISimpleGrain GetSimpleGrain()
        {
            if( MySimpleGrain == null )
                MySimpleGrain = SimpleGrainFactory.GetGrain((new Random()).Next(), "SimpleGrain");
