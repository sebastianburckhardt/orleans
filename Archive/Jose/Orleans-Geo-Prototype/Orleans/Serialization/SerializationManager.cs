//#define ASSUME_ARRAYS_ARE_IMMUTABLE
using System;
...
namespace Orleans.Serialization
{
    ...
    public static class SerializationManager
    {
        ...
        //private static readonly RuntimeTypeHandle stringTypeHandle = typeof(string).TypeHandle;
        ...
        static internal void Initialize(bool useStandardSerializer)
        {
            ...
            {
                CounterStorage store = CounterStorage.LogOnly;
                Copies = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_DEEPCOPIES, store);
                Serializations = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_SERIALIZATION, store);
                Deserializations = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_DESERIALIZATION, store);
                HeaderSers = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_SERIALIZATION, store);
                HeaderDesers = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_DESERIALIZATION, store);
                HeaderSersNumHeaders = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_SERIALIZATION_NUMHEADERS, store);
                HeaderDesersNumHeaders = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_DESERIALIZATION_NUMHEADERS, store);
                CopyTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_DEEPCOPY_MILLIS, store).AddValueConverter(TicksToMilliSeconds);
                SerTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_SERIALIZATION_MILLIS, store).AddValueConverter(TicksToMilliSeconds);
                DeserTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_DESERIALIZATION_MILLIS, store).AddValueConverter(TicksToMilliSeconds);
                HeaderSerTime = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_SERIALIZATION_MILLIS, store).AddValueConverter(TicksToMilliSeconds);
                HeaderDeserTime = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_HEADER_DESERIALIZATION_MILLIS, store).AddValueConverter(TicksToMilliSeconds);

                TotalTimeInSerializer = IntValueStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_TOTAL_TIME_IN_SERIALIZER_MILLIS, () =>
                        {
                ...
                CounterStorage storeFallback = CounterStorage.LogOnly;
                FallbackSerializations = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_SERIALIZATION, storeFallback);
                FallbackDeserializations = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_DESERIALIZATION, storeFallback);
                FallbackCopies = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_DEEPCOPIES, storeFallback);
                FallbackSerTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_SERIALIZATION_MILLIS, storeFallback).AddValueConverter(TicksToMilliSeconds);
                FallbackDeserTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_DESERIALIZATION_MILLIS, storeFallback).AddValueConverter(TicksToMilliSeconds);
                FallbackCopiesTimeStatistic = CounterStatistic.FindOrCreate(StatNames.STAT_SERIALIZATION_BODY_FALLBACK_DEEPCOPY_MILLIS, storeFallback).AddValueConverter(TicksToMilliSeconds);
            }
        }
        ...
        static SerializationManager()
        {
            ...
            logger = Logger.GetLogger("SerializationManager", Logger.LoggerType.Runtime);
            ...
            // Built-in handlers: F# types
            //Register(typeof(Microsoft.FSharp.Collections.FSharpList<>), BuiltInTypes.CopyGenericFSharpList, BuiltInTypes.SerializeGenericFSharpList,
            //         BuiltInTypes.DeserializeGenericFSharpList);
            ...
            Register(typeof(OrleansResponse), BuiltInTypes.CopyOrleansResponse, BuiltInTypes.SerializeOrleansResponse,
            ...
            Register(typeof(OrleansResponse));
            ...
            // Built-in handlers: sync objects
            //Register(typeof(SyncCounter), BuiltInTypes.SerializeMergingCounter, BuiltInTypes.DeserializeMergingCounter);
        }
        ...
        public static void Register(Type t, DeepCopier cop, Serializer ser, Deserializer deser, bool forceOverride)
        {
            ...
            lock (registeredTypes)
            {
                ...
                else
                {
                    ...
                    //if (registeredTypes.Count % 100 == 0)
                    //{
                    //    logger.Info("Registered type count is now {0}", registeredTypes.Count);
                    //}
                }
            }
        ...
        internal static void FindSerializationInfo(Assembly assembly)
        {
            ...
            // Skip assemblies not generated by the Orleans ClientGenerator tool
            //if (assembly.GetCustomAttributes(typeof(ClientProxyAttribute), false).Length == 0)
            //{
            //    return;
            //}
                    ...
                    else if (!systemAssembly)
                    {
                                ...
                                && !type.Namespace.StartsWith("System.", StringComparison.Ordinal))))
                        {
                            if (type.GetCustomAttributes(typeof(RegisterSerializerAttribute), false).Length > 0)
                            {
                                ...
                                if (logger.IsVerbose3) logger.Verbose3("Running register method for type {0} from assembly {1}", type.Name,
                                ...
                                if (register != null)
                                {
                                    ...
                                    catch(Exception ex)
                                    {
                                        ...
                                        if (SafeFailSerializers.Contains(assembly.GetName().Name))
                                            logger.Error(ErrorCode.SerMgr_TypeRegistrationFailureIgnore, "Failure registering type " + type.OrleansTypeName() + " from assembly " + assembly.GetLocationSafe() + ". Ignoring it.", ex);
                                        else
                                            throw;
                                    }
                                }
                            ...
                            else
                            {
                                                        ...
                                                        BindingFlags.NonPublic))
                                {
                                    if (method.GetCustomAttributes(typeof(CopierMethodAttribute), true).Length > 0)
                                    {
                                        ...
                                        method.GetCustomAttributes(typeof(SerializerMethodAttribute), true).Length >
                                        0)
                                    {
                                        serializer = method;
                                    }
                                    else if (
                                        method.GetCustomAttributes(typeof(DeserializerMethodAttribute), true).
                                            Length > 0)
                                    {
                                        deserializer = method;
                                    }
                                ...
                                if ((serializer != null) && (deserializer != null) && (copier != null))
                                {
                                    ...
                                    if (logger.IsVerbose3)
                                        logger.Verbose3("Loaded serialization info for type {0} from assembly {1}",
                                                        type.Name, assembly.GetName().Name);
                                }
                                ...
                                else if (!type.IsSerializable)
                                {
                                    ...
                                    {
                                        if (iface.IsGenericType && (iface.GetGenericTypeDefinition().Equals(typeof(IComparer<>))
                                            || iface.GetGenericTypeDefinition().Equals(typeof(IEqualityComparer<>))))
                                        {
                                            comparer = true;
                                            break;
                                        }
        ...
        public static object DeepCopy(object original)
        {
            ...
            //if (UseStandardSerializer)
            //{
            //    copy = FallbackSerializationDeepCopy(original);
            //}
            //else
            {
                SerializationContext.Current.Reset();
                copy = DeepCopyInner(original);
                SerializationContext.Current.Reset();
            }
            ...
            {
                timer.Stop();
                //var typeName = original.GetType().OrleansTypeName();
                //CounterStatistic.FindOrCreate("Serialization." + typeName + ".Copies").Increment();
                //CounterStatistic.FindOrCreate("Serialization." + typeName + ".CopyMillis").IncrementBy(timer.ElapsedTicks);
        ...
        public static object DeepCopyInner(object original)
        {
...
#if ASSUME_ARRAYS_ARE_IMMUTABLE
            ...
            if (copier != null)
            {
                ...
                // TODO: Once generated serializers are rebuilt, this line is redundant
        ...
        public static void SerializeInner(object obj, BinaryTokenStreamWriter stream, Type expected)
        {
            ...
            if (stream.TryWriteSimpleObject(obj))
            {
                return;
            }
            ...
            // We could have treated strings up above in the IdCategory switch, but by doing them here we avoid
            // serializing duplicate copies in case that the same string is sent multiuple times.
            // Since strings are immutable, this is not required for correctness, but may compress the serialized result.
            //if (t.TypeHandle.Equals(stringTypeHandle))
            //{
            ...
            if (t.IsArray)
            {
                ...
                {
                    FallbackSerializer(obj, stream);
                    //throw new ArgumentException("No serializer found for array with elements of type " + et.OrleansTypeName()
        ...
        public static object Deserialize(Type t, BinaryTokenStreamReader stream)
        {
            ...
            //if (UseStandardSerializer)
            //{
            //    result = FallbackDeserializer(stream);
            //}
            //else
            {
        ...
        public static object DeserializeInner(Type expected, BinaryTokenStreamReader stream)
        {
            ...
            //    return null;
            //}
            //if (token == SerializationTokenType.True)
            //{
            //    return true;
            //}
            //if (token == SerializationTokenType.False)
            //{
            //    return false;
            //}
            ...
            {
                //resultType = BinaryTokenStreamReader.CheckSpecialTypeCode(token);
                //if (resultType == null)
                //{
            ...
            // Handle primitives, which can never be back-referenced (except for strings)
            //switch (Type.GetTypeCode(resultType))
            //{
            //    case IdCategory.Boolean:
            //        // This should never happen -- booleans get serialized as True or False
            //        throw new SerializationException("Invalid primitive Boolean type specifier");
            //    case IdCategory.Byte:
            //        return stream.ReadByte();
            //    case IdCategory.SByte:
            //        return stream.ReadSByte();
            //    case IdCategory.Int16:
            //        return stream.ReadShort();
            //    case IdCategory.Int32:
            //        return stream.ReadInt();
            //    case IdCategory.Int64:
            //        return stream.ReadLong();
            //    case IdCategory.UInt16:
            //        return stream.ReadUShort();
            //    case IdCategory.UInt32:
            //        return stream.ReadUInt();
            //    case IdCategory.UInt64:
            //        return stream.ReadULong();
            //    case IdCategory.Single:
            //        return stream.ReadFloat();
            //    case IdCategory.Double:
            //        return stream.ReadDouble();
            //    case IdCategory.Decimal:
            //        return stream.ReadDecimal();
            //    case IdCategory.Char:
            //        return stream.ReadChar();
            //    case IdCategory.String:
            //        var str = stream.ReadString();
            //        //str = String.Intern(str);
            //        DeserializationContext.Current.RecordObject(start, str);
            //        return str;
            //    case IdCategory.DateTime:
        ...
        private static Dictionary<string, object> DeserializeMessageHeaderDictHelper(BinaryTokenStreamReader stream)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                //result[key] = DeserializeMessageHeaderHelper(stream);
        ...
        //internal static bool IsRecognized(Type t)
        //{
        //    if (t.IsOrleansShallowCopyable())
        //    {
        //        return true;
        ...
        //    if (t.IsArray)
        //    {
        //        return IsRecognized(t.GetElementType());
        //    }

        //    if (t.IsGenericType && !t.IsGenericTypeDefinition)
        //    {
        //        return IsRecognized(t.GetGenericTypeDefinition()) &&
        //               t.GetGenericArguments().All(param => IsRecognized(param));
        //    }
        ...
        private static bool HasOrleansSerialization(Type t)
        {
            switch (Type.GetTypeCode(t))
            {
                ...
                default:
                    ...
                    return t.Equals(typeof(string)) || serializers.ContainsKey(t.TypeHandle);
            }
        }
        ...
        internal static Type ResolveTypeName(string typeName)
        {
            ...
            if (typeName[typeName.Length - 1] == ']')
            {
                ...
                string baseName = typeName.Substring(0, j);
                Type baseType = ResolveTypeName(baseName);
                if (rank == 1)
                {
            ...
            if (i >= 0)
            {
                ...
                string baseName = typeName.Substring(0, i) + "'";
                ...
                Type baseType = ResolveTypeName(baseName + typeArgs.Count);
        ...
        internal static object RoundTripSerializationForTesting(object source)
        {
            ...
            object copy = DeserializeFromByteArray<object>(data);
            return copy;
        }
