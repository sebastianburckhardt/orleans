...
namespace UnitTests.DirectoryTests
{
    ...
    {
        //[TestMethod]
        //public void StartMultipleSilos()
        //{
        //    var silos = this.StartAdditionalOrleansRuntimes(5);
        //    MockSiloStatusOracleController controller = GloballySharedMemoryConnection.GetController();
        //    controller.CreateView("island1");
        //    controller.CreateView("island2");
        //    //
        //    var silo3 = silos[3];
        //    // set first two in one view
        //    controller.SetView("island1", silos.Take(2).Select(silo => silo.Silo.SiloAddress).ToArray());
        //    var p1 = controller.GetProvider("island1");
        //    p1.SetSiloStatus(silo3.Silo.SiloAddress, SiloStatus.Dead);

        //    //silo3.Silo.LocalGrainDirectory.GetLocalDataForGrain(); ;

        //    // create another view for last two
        //    controller.SetView("island2", silos.Skip(2).Select(silo => silo.Silo.SiloAddress).ToArray());
        //    var p2 = controller.GetProvider("island1");
        //    p1.SetSiloStatus(silo3.Silo.SiloAddress, SiloStatus.Active);
        ...
        public DirectoryTests()
            : base(new Options { DirectoryReplicationFactor = 3 })
        { 
        ...
        internal void BackupVerificationScenario(List<GrainId> grains, int nTh =0, bool shouldKill = false)
        {
            foreach (var grainId in grains) // for each grain that we created.
            {
                ValidatePrimaryForGrainOnAllSilos(grains);
                var primaryForGrain = GetSiloForAddress(Primary.Silo.LocalGrainDirectory.GetPrimaryForGrain(grainId));
                var backup = GetSiloForAddress(primaryForGrain.Silo.LocalGrainDirectory.GetReplicasForGrain(grainId).Skip(nTh).First());
        ...
        internal void BackupBatchVerificationScenario(List<GrainId> grains, bool stopPrimary, bool shouldKill = false, params int[] backupsToKill )
        {
            ...
            foreach (var grainId in grains) // for each grain that we created.
            {
                ...
                {
                    var backup = GetSiloForAddress(primaryForGrain.Silo.LocalGrainDirectory.GetReplicasForGrain(grainId)[i]);
                    BounceRuntime(shouldKill, backup);
                }
                if (stopPrimary)
                {
        ...
        private void ValidatePrimaryForGrainOnAllSilos(IEnumerable<GrainId> grainIds)
        {
            foreach (var siloHandle in GetActiveSilos()) // do this for each silo
            {
                foreach (var grainId in grainIds)
                {
                    ...
                    // 5. Find backups
                    var backups = siloHandle.Silo.LocalGrainDirectory.GetReplicasForGrain(grainId);
