...
namespace GeoOrleans.Benchmarks.LeaderboardPileus.Benchmark
{
    ...
    public class SequencedLeaderboard : IScenario
    {
        ...
        public async Task<string> RobotScript(IRobotContext context, int robotnumber, string parameters)
        {
            ...
            string result = await context.ServiceRequest(new HttpRequestSequencedLeaderboard(runTime * robotnumber + totOps, false, dummyGrain));
    ...
    public class HttpRequestSequencedLeaderboard : IHttpRequest
    {
        ...
        public string Signature
        {
            get
            {
                ...
                {
                    return "GET leaderboardpileus?reqtype=" + Convert.ToInt32(requestType) + "&" + "numreq=" + numReq + "&rep=1" + "&dummy=" + dummyGrain;
                }
                else
                {
                    return "GET leaderboardpileus?reqtype=" + Convert.ToInt32(requestType) + "&" + "numreq=" + numReq + "&score=" + score.ToString() + "&rep=1" + "&dummy=" + dummyGrain;
                }
            }
        }
        ...
        public async Task<string> ProcessRequestOnServer()
        {
            ...
            if (dummyGrain == 1)
            {
                ...
                using (new GeoOrleans.Runtime.Common.TraceInterval("Leaderboard FE - dummy - getgrain", 0))
                {
                    leaderboard = DummySequencedLeaderboardGrainFactory.GetGrain("test");
                }
                if (requestType == LeaderboardRequestT.GET_SYNC)
                {
                    //              Console.Write("Get Cuurent \n");

                    using (new GeoOrleans.Runtime.Common.TraceInterval("Leaderboard FE - dummy - get - sync", 1))
                    {
            ...
            else
            {
                ...
                {
                    leaderboard = SequencedLeaderboardGrainFactory.GetGrain("hello");
         //           leaderboard = SequencedLeaderboardGrainFactory.G
                }
                if (requestType == LeaderboardRequestT.GET_SYNC)
                {
                    //              Console.Write("Get Cuurent \n");
                    ...
                    {
                        scores = await leaderboard.GetExactTopTen("hello");
                        posts = LeaderboardPileus.Interfaces.Score.PrintScores(scores);
                        if (posts == null) posts = "";
                    //    posts = "";
                    }
                    //            Console.Write("{0}\n", posts);
