...
namespace UnitTests.StorageTests
{
    ...
    public class PersistenceGrainTests_Local : UnitTestBase
    {
        ...
        public async Task Persistence_Grain_Activate_StoredValue()
        {
            ...
            SetStoredValue(providerName, id, "Field1", initialValue);
        ...
        public async Task Persistence_Grain_ReRead()
        {
            ...
            storageProvider.SetValue(id, "Field1", 42); // Update state data behind grain
        ...
        {
            Guid id = Guid.NewGuid();
            IMemoryStorageTestGrain grain = MemoryStorageTestGrainFactory.GetGrain(id);
        ...
        public async Task Persistence_Provider_Error_BeforeRead()
        {
            ...
            storageProvider.SetValue(id, "Field1", expectedVal); // Update state data behind grain
        ...
        public async Task Persistence_Provider_Error_AfterRead()
        {
            ...
            int newVal = 53;
            storageProvider.SetErrorInjection(ErrorInjectionPoint.None);
            storageProvider.SetValue(id, "Field1", newVal); // Update state data behind grain
        ...
        {
            Guid id = Guid.NewGuid();
            IPersistenceUserHandledErrorGrain grain = PersistenceUserHandledErrorGrainFactory.GetGrain(id);
        ...
        public async Task Persistence_Provider_Loop_Read()
        {
            ...
            for (int i = 0; i < numIterations; i++)
            {
                ...
                Guid id = grain.GetPrimaryKey();
                SetStoredValue("test1", id, "Field1", expectedVal); // Update state data behind grain
        ...
        public async Task Persistence_Provider_Error_Stress_Read()
        {
            ...
            for (int i = 0; i < numIterations; i++)
            {
                ...
                Guid id = grain.GetPrimaryKey();
                SetStoredValue(ErrorInjectorStorageProvider, id, "Field1", expectedVal); // Update state data behind grain
        ...
        public async Task MemoryStore_UserGrain_Read_Write()
        {
            ...
            string readName = await grain.Name;
            ...
            var readName1 = await friend1.Name;
            var readName2 = await friend2.Name;
            ...
            Assert.AreEqual(name1, await friends[0].Name, "GetFriends - Friend #1 Name");
            Assert.AreEqual(name2, await friends[1].Name, "GetFriends - Friend #2 Name");
        }
        ...
        private void SetStoredValue(string providerName, Guid id, string fieldName, int newValue)
        {
            List<SiloHandle> silos = GetActiveSilos().ToList();
            foreach (var siloHandle in silos)
            {
                ...
                provider.SetValue(id, fieldName, newValue);
            }
        }
        ...
        private MockStorageProvider FindStorageProviderInUse(string providerName)
        {
    ...
    public class PersistenceGrainTests_AzureStore : UnitTestBase
    {
        ...
        public void Persistence_Perf_Write_Reread()
        {
            ...
            RunPerfTest(n, testName + "--ReRead", target,
                grainNoState => grainNoState.LastEchoAsync,
        ...
        public void AzureStore_ConvertToFromStorageFormat_GrainReference()
        {
            ...
            GrainStateContainingGrainReferences initialState = new GrainStateContainingGrainReferences { Grain = grain };
            ...
            GrainStateContainingGrainReferences convertedState = new GrainStateContainingGrainReferences();
        ...
        private static void RunIterations(string testName, int n, Func<int, Task> action)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                {
                    Task.WaitAll(promises.ToArray(), AzureTableDefaultPolicies.TableCreation_TIMEOUT);
                    promises.Clear();
