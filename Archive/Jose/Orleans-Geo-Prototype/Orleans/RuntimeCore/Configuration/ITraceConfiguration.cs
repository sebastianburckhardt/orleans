...
namespace Orleans
{
    ...
    public interface ITraceConfiguration
    {
        ...
        /// </summary>
        OrleansLogger.Severity DefaultTraceLevel { get; set; }
        /// <summary>
        ...
        /// </summary>
        List<Tuple<string, OrleansLogger.Severity>> TraceLevelOverrides { get; }
        /// <summary>
    ...
    public class MessagingConfiguration : IMessagingConfiguration
    {
        ...
        internal static readonly int DEFAULT_MAX_FORWARD_COUNT = 2;
        private static readonly bool DEFAULT_RESEND_ON_TIMEOUT = false;
        private static readonly bool DEFAULT_USE_STANDARD_SERIALIZER = false;
        ...
        private static readonly int DEFAULT_BUFFER_POOL_BUFFER_SIZE = 4 * 1024;
        private static readonly int DEFAULT_BUFFER_POOL_MAX_SIZE = 10000;
        private static readonly int DEFAULT_BUFFER_POOL_PREALLOCATION_SIZE = 250;
        private static readonly bool DEFAULT_DROP_EXPIRED_MESSAGES = true;
        private static readonly double DEFAULT_ERROR_INJECTION_RATE = 0.0;
        private static readonly bool DEFAULT_USE_MESSAGE_BATCHING = false;
        private static readonly int DEFAULT_MAX_MESSAGE_BATCH_SIZE = 10;

        private bool isSiloConfig;
        ...
        public override string ToString()
        {
            ...
            if (isSiloConfig)
            {
                ...
                //sb.AppendFormat("       Error injection:").AppendLine();
                //sb.AppendFormat("           RejectionInjectionRate: {0:F2}", RejectionInjectionRate).AppendLine();
                //sb.AppendFormat("           MessageLossInjectionRate: {0:F2}", MessageLossInjectionRate).AppendLine();
            }
            return sb.ToString();
        }
        ...
        internal virtual void Load(XmlElement child)
        {
            ...
            if (isSiloConfig)
            {
                ...
                //if (child.HasAttribute("RejectionInjectionRate"))
                //{
                //    RejectionInjectionRate = ConfigUtilities.ParseDouble(child.GetAttribute("RejectionInjectionRate"),
                //                                              "Invalid double value for the RejectionInjectionRate attribute on the Messaging element");
                //}
                //if (child.HasAttribute("MessageLossInjectionRate"))
                //{
                //    MessageLossInjectionRate = ConfigUtilities.ParseDouble(child.GetAttribute("MessageLossInjectionRate"),
                //                                              "Invalid double value for the MessageLossInjectionRate attribute on the Messaging element");
                //}
            }
        }
    }
    ...
    public class ProviderCategoryConfiguration
    {
        ...
        public Dictionary<string, IProviderConfiguration> Providers { get; set; }
        ...
        public override string ToString()
        {
            ...
            {
                sb.AppendFormat("           {0}", kv.Value.ToString()).AppendLine();
            }
            return sb.ToString();
        }
    }
    ...
    public class ProviderConfiguration : IProviderConfiguration
    {
        ...
        private List<ProviderConfiguration> _childConfigurations;
        private List<IOrleansProvider> _childProviders;
        [NonSerialized]
        ...
        /// </summary>
        public Dictionary<string, string> Properties { get { return new Dictionary<string, string>(_properties); } }
        ...
        public ProviderConfiguration(Dictionary<string, string> properties, string type, string name)
        {
        ...
        internal ProviderConfiguration(Dictionary<string, string> properties, List<IOrleansProvider> childProviders)
        {
            _properties = properties;
        ...
        internal void Load(XmlElement child, Dictionary<string, IProviderConfiguration> alreadyLoaded, XmlNamespaceManager nsManager)
        {
            _childConfigurations = new List<ProviderConfiguration>();
            ...
            LoadProviderConfigurations(child, nsManager, alreadyLoaded, c => _childConfigurations.Add(c));
        }

        internal static void LoadProviderConfigurations(XmlElement root, XmlNamespaceManager nsManager, 
            Dictionary<string, IProviderConfiguration> alreadyLoaded, Action<ProviderConfiguration> add)
        {
        ...
        internal void SetProviderManager(IProviderManager providerManager)
        {
            _providerManager = providerManager;
            foreach(var child in _childConfigurations)
                child.SetProviderManager(providerManager);
        }
        ...
        public override string ToString()
        {
            ...
            return string.Format("Name={0}, Type={1}, Properties={2}", Name, Type, Utils.IEnumerableToString(_properties.Keys));
        }

        /// <summary>
        /// Children providers of this provider. Used by hierarchical providers.
        /// </summary>
        public List<IOrleansProvider> Children
        {
            get
            {
                if (_childProviders != null)
                    return new List<IOrleansProvider>(_childProviders); //clone

                var list = new List<IOrleansProvider>();

                if (_childConfigurations.Count == 0)
                    return list; // empty list

                foreach (var config in _childConfigurations)
                    list.Add(_providerManager.GetProvider(config.Name));
                
                _childProviders = list;
                return new List<IOrleansProvider>(_childProviders); // clone
            }
        }
    }
